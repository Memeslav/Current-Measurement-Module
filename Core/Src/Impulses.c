...
{
	uint32_t start_time = (uint32_t)(impulse_data.impulse.start_time_hi << 16) | impulse_data.impulse.start_time_lo;
	uint32_t end_time = (uint32_t)(registers.unixtime.hi << 16) | registers.unixtime.lo;

	uint32_t impulse_duration = end_time - start_time;
